      ...
      <Modal open={open} onClose={handleClose}>
        <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 600, bgcolor: 'background.paper', borderRadius: '8px', boxShadow: 24, p: 4 }}>
          ...
          <Form onSubmit={handleSignUp}>
            ...
            <FormField>
              <label>Role</label>
              <Dropdown placeholder='Select Role' options={roleOptions} className="form-dropdown" value={role} onChange={(e, { value }) => setRole(value)} selection />
            </FormField>
            <Button type='submit' className='submit'>Submit</Button>
            <div className="login" style={{ fontSize: '1.3rem' }}>
              Already have an account?  
              <span onClick={() => { handleClose(); handleOpenSignIn(); }} className="login-navigation" style={{ fontSize: '1.5rem', cursor: "pointer", color: "#cc0000", textDecoration: "underline" }}>Login</span>
            </div>
          </Form>
        </Box>
      </Modal>
